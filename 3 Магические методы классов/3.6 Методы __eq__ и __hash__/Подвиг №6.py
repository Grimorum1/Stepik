...
class ShopItem:
    ...
    def __eq__(self, other):
        return (self.name.lower()==other.name.lower() and self.price==other.price and self.weight==other.weight)



lst_in = [i for i in x.split("\n")]
shop_items = {}
for i in lst_in:
    a = i.split(":")
    p = ShopItem(a[0], a[1].split()[0], a[1].split()[1])
    if p not in shop_items:
        shop_items[p]=[p, 1]
    else:
        shop_items[p][1]+=1
