...
СМОТРИ КАРТИНКУ TableValues.png
...
class TableValues:
    ...
    def __init__(self, rows, cols, type_data=int):
        ...
        self.spisok = [[Cell(0) for j in range(self.cols)]for i in range(self.rows)]

    def __getitem__(self, item):
        if item[0]>self.rows or item[1]>self.cols:
            raise IndexError('неверный индекс')
        return self.spisok[item[0]][item[1]].data
    ...
    def __setitem__(self, key, value):
        ...
        if type(value)!=self.type_data:
            raise TypeError('неверный тип присваиваемых данных')
        self.spisok[key[0]][key[1]].data = value
